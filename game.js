...
function loadImages() {
  ASSETS.girl.src = "sprites/girl_spritesheet.png";
  ASSETS.boy.src = "sprites/boy_spritesheet.png";
  ASSETS.girlSpin.src = "sprites/girl_spin_animation.png";
  ASSETS.boySpin.src = "sprites/boy_spin_animation.png";
...
function saveLeaderboard() {
  try {
    localStorage.setItem("cloud9_bounce_lb", JSON.stringify(leaderboard));
  } catch (e) {}
}
  ...
  if (state === STATE.TITLE) {
    ...
    if (e.code === "Space" || e.code === "Enter") {
      ...
      if (!globalInputPressed) {
        globalInputPressed = true;
        globalInputJustPressed = true;
      }
    }
...
function drawPlayer(x, y, expression, rot, scale, pose, stretchX, stretchY, playerIndex = 0, actionResult = '') {
  ...
  // 2px should be enough to avoid interpolation artifacts without losing content.
  const padX = 2; 
  const padY = 2; 
  ...
  if (actionResult === 'perfect' && pose === 'bounce') {
      // Peak analysis detected 4 frames for spin animations!
      // 1024 / 4 = 256px width per frame.
      const spinFrameCount = 4;
      const spinFrameW = 1024 / spinFrameCount; // 256
...
function drawCloudPuff(x, y, type = 0, squish = 0, flash = 0, scale = 1, isPerfect = false) {
  ...
  // 1024x1024 sheet, 2x2 grid.
  // Each cell is 512x512.
  const frameW = 512; 
  const frameH = 512; 
  const drawSize = 120; // Slightly larger to fill space
  ...
  // Center: -drawSize/2
  // Adjust Y offset slightly to align with feet
...
function drawStormPuff(x, y, type = 0, squish = 0, flash = 0, scale = 1, isPerfect = false) {
  // Boss is basically Thunder Cloud but bigger
